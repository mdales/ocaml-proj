...
let test_inverse_round_trip_transform () =
    ...
    let restored = Proj.Transformation.transform ~direction:Inverse crs mollweide in
...
let () =
    ...
    [
        ...
        Alcotest.test_case "test inverse round trip" `Quick test_inverse_round_trip_transform;
        Alcotest.test_case "test manual round trip" `Quick test_manual_round_trip_transform;
        ])
    ]
